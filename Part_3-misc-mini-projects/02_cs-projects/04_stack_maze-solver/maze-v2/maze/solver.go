...
func expandNode(node Node, maze Matrix, frontier *stack.Stack[Node], exploredSet *ExploredSet) {
...
func buildSolution(node Node) (solution []string) {
	reversedMoves := stack.New[string]()

	for node.Parent != nil {
		reversedMoves.Push(fmt.Sprintf("from (row=%d, col=%d), move (%d, %d) to (%d, %d) (cost=%d)", node.Parent.State.Row, node.Parent.State.Col, node.Action.RowMove, node.Action.ColMove, node.State.Row, node.State.Col, node.Cost))
		node = *node.Parent
	}
