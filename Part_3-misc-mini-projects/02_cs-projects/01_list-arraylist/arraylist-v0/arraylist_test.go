...
func TestInsertHappyPath(t *testing.T) {
	tests := map[string]struct {
		prevList   []Elem // state of the list before test (as slice)
		elem       Elem   // elem to insert
		pos        int    // pos on which elem will be inserted
		wantedList []Elem // state of the list after test (as slice)
	}{
	...
	for testName, testCase := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			assertListLen(t, l.length, len(testCase.prevList)+1)
		})
	}
}
...
func TestRemoveHappyPath(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Remove from list with four elems, pos 0": {
			prevList:   []Elem{"a", "b", "c", "d"},
			pos:        0,
			wantedList: []Elem{"b", "c", "d"},
		},
		"Remove from list with four elems, pos 1": {
			prevList:   []Elem{"a", "b", "c", "d"},
			pos:        1,
			wantedList: []Elem{"a", "c", "d"},
		},
		"Remove from list with four elems, pos 2": {
			prevList:   []Elem{"a", "b", "c", "d"},
			pos:        2,
			wantedList: []Elem{"a", "b", "d"},
		},
...
func TestLenProperties(t *testing.T) {
	assertion := func() (err bool) {
		...
		// Set up the initial slice
		numElems := rand.Intn(MaxElems)
		slice := make([]Elem, numElems)
		for i := 0; i < numElems; i++ {
			slice[i] = Elem(fmt.Sprintf("%d", rand.Int()))
		}
...
func assertListContents(t testing.TB, got *List, wanted []Elem) {
	t.Helper()
...
func toSlice(l *List) []Elem {
	s := []Elem{}
	l.Traverse(func(e Elem) {
		s = append(s, e)
	})
	return s
}

func toList(t testing.TB, elems []Elem) (l *List) {
	t.Helper()
	l = NewList()
