...
func TestGameAskHappyPathSimple(t *testing.T) {
	...
	g, _ := New(strings.NewReader(input), []string{"Hello"}, 3)
	actual := g.ask()
...
func TestGameAskHappyPath(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			g, _ := New(strings.NewReader(tc.input), []string{tc.input}, 3)
...
func TestValidateGuess(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			g, _ := New(nil, []string{"Hello"}, 3) // We need g only to call validateGuess
